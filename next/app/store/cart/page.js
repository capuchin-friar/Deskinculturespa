  ...
  async function setQty(cartLineId, next) {
    ...
    try {
      ...
      let {
        data,
        status
      } = await baseApi.patch("/cart/edit", { id: Number(cartLineId), qty: q });

      if (!data.success) {
        setItems(prev);
        const data = await res.json().catch(() => ({}));
        setActionError(typeof data.message === "string" ? data.message : "Could not update quantity.");
      } else {
        setActionError("");
      }
    } catch {
  ...
  async function removeLine(cartLineId) {
    ...
    try {
      ...
      let {
        data,
        status
      } = await baseApi.delete("/cart/delete", { data: {id: Number(cartLineId)} });
      if (!data.success) {
        setItems(prev);
        setActionError(typeof data.message === "string" ? data.message : "Could not remove item.");
      } else {
        setActionError("");
      }
    } catch {
                  ...
                  <article key={String(row.id)} className="crt-line">
                    ...
                    <div className="crt-line__controls">
                      <label className="crt-qty-label">
                        ...
                        >
                          {Array.from({ length: row.stock }, (_, i) => i + 1).map((n) => (
